...
Данные из "сырого" транзакционного вида проходят определнный пайплайн по подготовке. Отбросив служебные __DAG__'и, первым запускается _Parser_, rоторый параллельно отсылает, добавляет данные в __PostgreSQL__. Далее отрабатывает _Send_Elastic_, который необходимую информацию из персистентного хранилища складывает в __ElastcSearch__. После всего этого, начинаются непосредственные вычисления, расчитываются __MASK__ для чековых данных, нужны они непосредственно для обучения и валидации модели. Пару слов о метрике, которую мы использовали,
